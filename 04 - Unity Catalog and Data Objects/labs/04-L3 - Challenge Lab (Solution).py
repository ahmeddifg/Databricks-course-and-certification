...
# MAGIC %md
# MAGIC # 🏆 Lab 04-L3 · Challenge Lab — Unity Catalog & Data Objects — ✅ SOLUTION
...
run_sql(f"CREATE SCHEMA IF NOT EXISTS {CH} COMMENT 'ShopWave sales mart'")
run_sql(f"USE CATALOG {LAB_CATALOG}")
run_sql(f"USE SCHEMA {MART_SCHEMA}")
# From here on, %sql cells can use short names such as  products  or  v_category_bands
...
# DBTITLE 1,Task 2 · SOLUTION (no LOCATION => managed table)
run_sql(f"""
    CREATE OR REPLACE TABLE {CH}.products AS
    SELECT product_id, title, category, CAST(price AS DECIMAL(10,2)) AS price
    FROM read_files('{dataset_path}/products-csv', format => 'csv', header => true, delimiter => ';')""")
# read_files() needs a literal path, so Python + f-string is the easiest way to use the dataset_path variable
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE FUNCTION price_band(price DOUBLE)
# MAGIC RETURNS STRING
# MAGIC COMMENT 'budget < 50 <= standard < 200 <= premium'
# MAGIC RETURN CASE WHEN price < 50  THEN 'budget'
# MAGIC             WHEN price < 200 THEN 'standard'
# MAGIC             ELSE 'premium' END
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE VIEW v_category_bands AS
# MAGIC SELECT category, price_band(price) AS band, count(*) AS products
# MAGIC FROM products
# MAGIC GROUP BY category, price_band(price)
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMP VIEW tv_premium AS
# MAGIC SELECT * FROM products WHERE price_band(price) = 'premium'
...
run_sql(f"CREATE VOLUME IF NOT EXISTS {CH}.exports")
(spark.table(f"{CH}.v_category_bands")
      .coalesce(1)                                   # one CSV file is nicer for people downloading it
      .write.mode("overwrite").option("header", "true")
      .csv(f"{EXPORTS}/category_bands"))
display(spark.sql(f"LIST '{EXPORTS}/category_bands'"))
...
display(run_sql(f"SHOW TABLES DROPPED IN {CH}"))     # the dropped table is listed with its tableId
run_sql(f"UNDROP TABLE {CH}.products")               # managed table, within the 7-day recovery window
display(spark.table(f"{CH}.v_category_bands"))       # the view works again - it only stores the query
...
answer_task8 = "B"
# External table = Unity Catalog manages only the metadata. DROP TABLE removes the metadata; the files at the
# LOCATION are untouched (you delete them yourself if you want). D is what happens to a UC MANAGED table (files purged
# after the recovery window); A is the legacy Hive-metastore managed-table behaviour. CASCADE is for schemas/catalogs.
...
answer_task9 = "B"
# A two-part name is schema.object and is resolved in the CURRENT CATALOG (prod) -> prod.finance.budget.
# USE SCHEMA only matters for one-part names.
